...
function buildHeuristicInsights(run, artifact, pageResults = []) {
  if (run.scanMode === "site") {
    ...
    if ((run.summary?.csp?.count || 0) > 0) {
      summary.push(
        `Detected ${run.summary.csp.count} CSP console violation(s) across ${run.summary.csp.pageCount || 0} page(s); review blocked scripts and third-party embeds.`
      );
    }
  ...
  if ((run.summary?.csp?.count || 0) > 0) {
    summary.push(
      `The browser console reported ${run.summary.csp.count} Content-Security-Policy violation(s) during the scan.`
    );
  }
